...
if 'IRIS_PARTY_CHEAT_CODE = "/iris123"' not in canon:
    canon = replace_once(
        canon,
...
intercept = '''    SpecialFollowersCanon.matchesPartyCheatCode(action)?.let { targetId ->\n      return applySpecialFollowerPartyCheat(legacy, state, targetId)\n    }\n'''
...
if "private fun applySpecialFollowerPartyCheat(" not in facade:
    ...
    facade = facade.replace(anchor, handler + anchor, 1)
facade_path.write_text(facade, encoding="utf-8")

test_path = TESTS / "SpecialFollowerShortcutTest.kt"
if not test_path.is_file():
    raise RuntimeError("Materialized SpecialFollowerShortcutTest.kt is missing")
...
for marker in (
    ...
    'incrementTurn = false',
    'exactSlashCodesResolveToTheRequestedFollowers',
):
    if marker not in combined:
        raise RuntimeError(f"Special follower shortcut/avatar contract missing: {marker}")

print("Iris/Syvial shortcut/avatar compatibility verified against materialized Kotlin authority.")
