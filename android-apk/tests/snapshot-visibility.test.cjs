...
function boot({unknown=false,unloaded=false,canvasContextMissing=false}={}){
 ...
 const document={readyState:'complete',head:{appendChild(el){styles.push(el.textContent);}},getElementById(id){return id==='snapshot'?box:null;},querySelectorAll(){return [];},createElement(tag){
  ...
  return {tagName:tag.toUpperCase(),style:{},dataset:{},className:'',complete:!unloaded,naturalWidth:0,naturalHeight:0,setAttribute(){},addEventListener(type,cb){if(type==='load')pending.push({el:this,cb});},set src(url){this.url=url;const m=geometry.assetMetric(url)||geometry.assetMetric('kai_snapshot_overlay.png');this.naturalWidth=m.width;this.naturalHeight=m.height;},get src(){return this.url;}};
 }};
 const ctx={document,console:{warn(){}},localStorage:{removeItem(){}},setTimeout,clearTimeout,Image:function(){throw Error('Detached image preload must not gate overlays');},state:{flags:{},combat:{active:true,participants:[{id:'kai'},{id:'lucia'}]}}};
 ctx.window=ctx;ctx.addEventListener=()=>{};
 vm.createContext(ctx);
 let js=source;if(unknown)js=js.replaceAll('file:///android_asset/kai_snapshot_overlay.png','file:///android_asset/unregistered.png');
...
test('generated bounds exist for all registered overlays and match PNG dimensions',()=>{
 ...
 const names=['kai_snapshot_overlay.png','kai_entity_overlay.png','lucia_entity_overlay.png',...fs.readdirSync(path.join(assets,'entity')).filter(n=>n.endsWith('.png')).map(n=>'entity/'+n)];
