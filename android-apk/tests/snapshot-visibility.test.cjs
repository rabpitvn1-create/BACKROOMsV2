...
function boot({unknown=false,unloaded=false,canvasContextMissing=false}={}){
 ...
 let js=source;if(unknown)js=js.replaceAll('file:///android_asset/cao_minh_entity_overlay.png','file:///android_asset/unregistered.png');
...
test('bundled exploration/combat/Entity render without any readable canvas or detached preloads',()=>{
 const r=boot();
 visible(r.elements.find(e=>String(e.url||'').includes('cao_minh_snapshot_overlay.png')));
 r.ctx.backroomSetCombatVisualActor(0,'deathmoth');
 visible(r.elements.find(e=>String(e.url||'').includes('cao_minh_entity_overlay.png')));
...
test('unknown sprite remains visible when canvas throws SecurityError',()=>{
 const r=boot({unknown:true});r.ctx.backroomSetCombatVisualActor(0,'deathmoth');visible(r.elements.find(e=>e.className.includes('snapshot-character')));assert.equal(r.reads(),1);
});
test('unknown sprite remains visible when canvas context is unavailable',()=>{
 const r=boot({unknown:true,canvasContextMissing:true});r.ctx.backroomSetCombatVisualActor(0,'deathmoth');visible(r.elements.find(e=>e.className.includes('snapshot-character')));
});
test('late image load aligns independently, without waiting for other characters',()=>{
 const r=boot({unloaded:true});r.ctx.backroomSetCombatVisualActor(0,'deathmoth');assert.ok(r.pending.length>0);
...
test('generated bounds exist for all registered overlays and match PNG dimensions',()=>{
 ...
 const names=['cao_minh_entity_overlay.png','cao_minh_snapshot_overlay.png','lucia_entity_overlay.png',...fs.readdirSync(path.join(assets,'entity')).filter(n=>n.endsWith('.png')).map(n=>'entity/'+n)];
