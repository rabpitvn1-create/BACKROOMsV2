...
# The An Nhiên cheat is already part of the checked-in Kotlin authority on the
# issue-43 materialized branch. Keep this historical patch idempotent so the old
# build chain can verify or backfill the same contract without duplicating it.
canon_path = CORE / "AnNhienCanon.kt"
canon = canon_path.read_text(encoding="utf-8")
if 'PARTY_CHEAT_CODE = "/annhien1234"' not in canon:
    canon = replace_once(
        canon,
...
intercept = '    if (AnNhienCanon.matchesPartyCheatCode(action)) return applyAnNhienPartyCheat(legacy, state)\n'
if intercept not in facade:
    facade = replace_once(
        facade,
...
if "private fun applyAnNhienPartyCheat(" not in facade:
    facade = replace_once(
        facade,
...
# Tests are also materialized; only backfill them when an older source snapshot is
# still feeding the compatibility chain.
test_path = ROOT / "app/src/test/java/com/rabpit/backroom/core/AnNhienFollowerTest.kt"
test = test_path.read_text(encoding="utf-8")
if "slashCheatInstantlyAddsAnNhienAndIsIdempotent" not in test:
    test = replace_once(
        test,
...
print("/annhien1234 compatibility verified: Kotlin Core owns the instant Party add and cap-safe behavior.")
