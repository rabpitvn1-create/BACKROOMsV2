...
helpers = r'''  private JSONObject debugJsonDiff(JSONObject before, JSONObject after) {
    ...
    for (String key : keys) {
      ...
      if (!a.equals(b)) {
        ...
        out.put(key, new JSONObject()
          .put("before", left == null ? JSONObject.NULL : left)
          .put("after", right == null ? JSONObject.NULL : right));
      }
    }
  ...
  private String debugProcessCombat(String stateJson, String actionKind, String action) throws Exception {
    ...
    debugEvent("combat", "core_request", turn, new JSONObject()
      .put("actionKind", actionKind == null ? "" : actionKind)
      .put("action", action == null ? "" : action)
      .put("stateBefore", before)
      .put("combatBefore", before.opt("combat"))
      .put("partyBefore", before.opt("party")));
    ...
    com.rabpit.backroom.core.RuntimeDebugLog.appendTurnStage(turn, "combat", new JSONObject()
      .put("request", new JSONObject().put("actionKind", actionKind).put("action", action).put("state", before))
      .put("result", result));
    return raw;
  }
...
if combat_call in text:
    text = text.replace(combat_call, "debugProcessCombat(stateJson, actionKind, action)")
# Restore the original call inside the wrapper if the global replacement touched it.
wrapper_start, wrapper_end = method_bounds(text, "  private String debugProcessCombat(")
wrapper = text[wrapper_start:wrapper_end]
wrapper = wrapper.replace("debugProcessCombat(stateJson, actionKind, action)", combat_call, 1)
text = text[:wrapper_start] + wrapper + text[wrapper_end:]

submit_start, submit_end = method_bounds(text, "    @JavascriptInterface public void submitTurn(String stateJson, String action) {")
...
local_new = '''          JSONObject debugBefore = new JSONObject(stateJson);
          ...
          debugEvent("game_core", "rule_request", debugTurn, new JSONObject().put("action", action).put("stateBefore", debugBefore));
...
submit = replace_once(
    submit,
    roll_anchor,
    roll_anchor + "          com.rabpit.backroom.core.RuntimeDebugLog.recordTurnStage(debugTurn, \"rolls\", rolls);\n          com.rabpit.backroom.core.RuntimeDebugLog.recordTurnStage(debugTurn, \"inputClassification\", new JSONObject().put(\"actionKind\", rolls.optString(\"actionKind\", \"\")).put(\"meta\", meta));\n",
    "roll trace",
)

writer_old = "          JSONObject generated = parseModelJson(generateText(writerPrompt(before, action, rolls, null)));\n"
writer_new = '''          String writerPromptText = writerPrompt(before, action, rolls, null);
...
submit = replace_once(
    submit,
    candidate_old,
    candidate_old + '''          com.rabpit.backroom.core.RuntimeDebugLog.recordTurnStage(debugTurn, "candidateInitial", candidateState);
...
repair_old = "            generated = parseModelJson(generateText(writerPrompt(before, action, rolls, hardIssues)));\n"
repair_new = '''            String repairPromptText = writerPrompt(before, action, rolls, hardIssues);
...
'''
submit = replace_once(submit, fallback_old, fallback_new, "fallback diagnostics trace")
...
'''
submit = replace_once(submit, core_old, core_new, "Core validated boundary trace")

final_emit = '          emit("backroomTurn", state.toString());\n'
submit = replace_once(
    submit,
    final_emit,
    '''          com.rabpit.backroom.core.RuntimeDebugLog.recordTurnStage(debugTurn, "finalCommittedState", state);
...
submit = replace_once(
    submit,
    error_old,
    '''          String debugError = e.getMessage() == null ? "Không thể xử lý lượt." : e.getMessage();
...
for marker in (
    ...
    if marker not in text:
        raise RuntimeError("turn trace marker missing: " + marker)

MAIN.write_text(text, encoding="utf-8")
print("Runtime turn trace installed: input/rolls/writer/ops/audit/repair/fallback/Core/final state/combat with full state deltas.")
