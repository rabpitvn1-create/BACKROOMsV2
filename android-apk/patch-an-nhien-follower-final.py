...
# Keep historical core/UI materialization, but retire section 7 Java gameplay authority.
...
def method_bounds(source: str, signature: str) -> tuple[int, int]:
    ...
    if start < 0:
        raise RuntimeError("Special follower bridge method missing: " + signature)
    ...
    while i < len(source):
        ...
        i += 1
    raise RuntimeError("Special follower bridge method closing brace missing")
...
if bridge_return not in method:
    ...
    if count < 1:
        raise RuntimeError("Special follower applyModelOperations return anchor missing")
...
print("Special follower gameplay authority bridged to Kotlin; historical patch retains core/UI compatibility only.")
