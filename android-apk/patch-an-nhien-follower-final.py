...
# but make the prose prompt anchor tolerant because earlier canon patches may already have rebuilt it.
...
optional_prompt = '''if prompt_marker in main:\n    main = main.replace(prompt_marker, prompt_extra, 1)\n'''
if code.count(strict_prompt) != 1:
    raise RuntimeError("An Nhien prompt compatibility anchor is not unique")
code = code.replace(strict_prompt, optional_prompt, 1)
exec(compile(code, str(SOURCE), "exec"), {"__name__": "__main__", "__file__": str(SOURCE)})

# Keep current 0.25% semantics explicit even when the historical prompt anchor was already rebuilt.
